...
grep -v '^#' "$urlList"| while IFS=$'\n' read urlToTest; do
	...
	#PHANTOMJS_EXECUTABLE=node_modules/phantomjs-prebuilt/bin/phantomjs node_modules/casperjs/bin/casperjs test tests/demo2/testsuite.js --baseUrl="$baseUrl" --url="$urlToTest" --testIdentifier="$testIdentifier" --urlCounter="$urlCounter" --finishedTestsCounter="$finishedTestsCounter" --disableNewBase="$disableNewBase" < /dev/null
